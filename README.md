...
| 功能 | 说明 | 实现位置 |
|---|---|---|
| 中文显示 | 12 个分字号字库（时间/任务名/正文/导航等按用途分别生成），覆盖 GB2312 一级汉字 | `ui/velatime_font_*.c` |
...
| 项 | 状态 |
|---|---|
| 网页本身 | ✅ **已完成**：Flask + SocketIO + SQLite，19 个页面 + 11 个 API，手机浏览器直接打开，不需要装 App |
...
> ⚠️ **当前的完成度**：网页本身已经可以完整运行（19 个页面 + 11 个 API），
...
| 扩展 | 状态 | 说明 |
|---|---|---|
| **手机网页控制台** | ✅ **界面模板已完成** | Flask + SocketIO + SQLite；19 个页面 + 11 个 API；手机浏览器直接打开，不需要装 App。课表与空档时间、课表 CSV 批量导入、待办管理、消息推送、快捷短语、主题、每日一句、倒计时、壁纸、数据导出与备份均已实现 |
